...
class LoggerService(
    ...
    fun startProcessing() {
        ...
        processingJob = CoroutineScope(Dispatchers.IO).launch {
            launch {
                redisService.activityLogFlow.collect { logEntry ->
                    processActivityLog(logEntry)
                }
            }
            
            launch {
                redisService.errorLogFlow.collect { logEntry ->
                    processErrorLog(logEntry)
                }
            }
        }
    ...
    private fun processActivityLog(logEntry: ActivityLogEntry) {
        if (shouldLogToConsole(logEntry.level)) {
            logger.info { "Активность: ${logEntry.event} | Пользователь: ${logEntry.userId ?: "неизвестен"} | Устройство: ${logEntry.deviceModel ?: "неизвестно"}" }
        }
    ...
    private fun writeToFile(type: String, content: String) {
        try {
            ...
            val dateStr = when (config.rotationPeriod.lowercase()) {
    ...
    fun logError(
        ...
        val previousActivities = if (userId != null) {
    ...
    fun getActivityLogs(limit: Int = 100, offset: Int = 0, userId: String? = null): List<ActivityLogEntry> {
