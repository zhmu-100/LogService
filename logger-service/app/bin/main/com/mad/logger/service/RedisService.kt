...
class RedisService(private val config: RedisConfig) {
    ...
    private fun setupSubscriptions() {
        ...
        logger.info { "Подписка на каналы Redis настроена: ${config.activityChannel}, ${config.errorChannel}" }
    }
    ...
    fun getErrorLogs(limit: Int = 100, offset: Int = 0, userId: String? = null): List<ErrorLogEntry> {
        return try {
            jedisPool.resource.use { jedis ->
                ...
                keys.mapNotNull { key ->
                    jedis.get(key)?.let { json.decodeFromString<ErrorLogEntry>(it) }
                }
            }
        } catch (e: Exception) {
